...
The Sensor Integrity Detection (SID) v6 logic looks for sustained glucose-deviation events that suggest a sensor problem (compression, calibration drift). We re-ran SID against each smoother's output to count surviving events. The headline numbers across the cohort are:
